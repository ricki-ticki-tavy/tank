...
class ACROBOTIC_SSD1306 {
  public:
    uint8_t m_font_first_char_code = 32;  // first symbol, containing in font.
    uint8_t m_font_last_char_code = 127;  // last symbol, containing in font.
    ...
    void drawBitmapRam(unsigned char *bitmaparray, int bytes);
    ...
    void setInverseLOutput(uint8_t inverted);
  ...
  private:
    ...
    uint8_t invertOutput = 0; //  инверсный вывод
};
